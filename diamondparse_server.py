#!/tscc/projects/ps-allenlab/projdata/common/programs/miniconda3/bin/python
...
#parser.add_argument('-hl', '--skip_hitless_queries', help='Skip queries lacking db hits', action='store_true')
parser.add_argument('-d', '--skip_dupes', help='Skip dupes from outfasta', action='store_true')
parser.add_argument('--suffix', help='Suffix for outfile', default='')

#parser.add_argument('-i', '--sseqid_idx', help='Zero-based index of the sseqid column', default=1)

args = parser.parse_args()
suffix = "_{}".format(args.suffix) if args.suffix != "" else ""
outpool = args.query.split(".")[0] + suffix
results = args.results.split(",")
#sseqidx = int(args.sseqid_idx)
print(results)
...
if args.pool in {True, "True", "true"}:
	...
	for query in sequence_dict:
		uniqseqs = set()
		#query.replace("/", "_") sometimes there are / in sequence names
		with open("{}{}{}_out.fasta".format(SORTDIR, query.replace("/", "_"), suffix), "w") as result: 
			for item in sequence_dict[query]:
				#print(item)
